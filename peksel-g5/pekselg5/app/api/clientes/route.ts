...
export async function GET() {
  ...
  const items = await db.all("SELECT * FROM clientes");
...
export async function PUT(request: Request) {
  ...
  const insertSql = `UPDATE clientes SET Nombre = ?, PrimerApellido = ?, SegundoApellido = ?, Correo = ?, Direccion = ?, Telefono = ? WHERE idCliente = ?`;
...
export async function DELETE(request: Request) {
  ...
  const insertSql = `DELETE FROM clientes WHERE idCliente = ?`;
